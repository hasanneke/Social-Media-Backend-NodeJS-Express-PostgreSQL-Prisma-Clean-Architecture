...
function drawBox({ title, width, height, str, horizontalPadding }) {
  ...
  const topLine = title ? grey(chars.topLeft + chars.horizontal) + " " + reset(bold(title)) + " " + grey(chars.horizontal.repeat(width - title.length - 2 - 3) + chars.topRight) + reset() : grey(chars.topLeft + chars.horizontal) + grey(chars.horizontal.repeat(width - 3) + chars.topRight);
  ...
  const mappedLines = lines.slice(-height).map((l) => {
    ...
    return `${grey(chars.vertical)}${" ".repeat(horizontalPadding)}${reset((0, import_cli_truncate.default)(l, width - 2))}${" ".repeat(
      paddingRight - horizontalPadding
    )}${grey(chars.vertical)}`;
  }).join("\n");
  return grey(topLine + "\n" + mappedLines + "\n" + bottomLine);
}
var import_cli_truncate, import_string_width, chars;
